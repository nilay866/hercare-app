              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  child: ExpansionTile(
                    ...
                    title: Text(
                      "Date: ${c['visit_date']} • Dr. ${c['doctor_name']}",
                    ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (c['prescriptions'] != null &&
                                (c['prescriptions'] as List).isNotEmpty)
                              ...((c['prescriptions'] as List).map(
                                (p) => Text(
                                  "💊 ${p['name']} - ${p['dosage']} (${p['timing']})",
                                ),
                              )),

                            const SizedBox(height: 8),
                            if (c['billing_items'] != null &&
                                (c['billing_items'] as List).isNotEmpty) ...[
                              const Divider(),
                              const Text(
                                "Invoice:",
                                style: TextStyle(fontWeight: FontWeight.bold),
                              ),
                              ...((c['billing_items'] as List).map(
                                (b) => Row(
                                  mainAxisAlignment:
                      ...
                      ButtonBar(
                        children: [
                          TextButton.icon(
                            ...
                            onPressed: () {
                              final rxText =
                                  "Prescription for Patient (Date: ${c['visit_date']})\n"
                                  "Dr. ${c['doctor_name']}\n\n"
                              ...
                              Share.share(
                                rxText,
                                subject: "Prescription - ${c['visit_date']}",
                              );
                            },
                          ),
                        ],
          ...
          ? FloatingActionButton(
              onPressed: () async {
                final result = await Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        AddConsultationScreen(patientId: widget.patientId),
                  ),
                );
...
class _AddConsultationScreenState extends State<AddConsultationScreen> {
  ...
  void _addBill() {
    ...
    setState(() {
      ...
      _billServiceCtrl.clear();
      _billCostCtrl.clear();
    });
  }

  double get _totalAmount =>
      _billingItems.fold(0.0, (sum, item) => sum + item['cost']);
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await ApiService.createConsultation(token: auth.token!, body: payload);
      if (mounted) Navigator.pop(context, true);
    } catch (e) {
      await AppLogger.log(
        'AddConsultationScreen._save',
        ...
        child: Column(
          ...
          children: [
            ...
            ..._prescriptions.map(
              (p) => ListTile(
                title: Text(p['name']),
                subtitle: Text("${p['dosage']} - ${p['timing']}"),
                trailing: IconButton(
            ...
            Row(
              children: [
                Expanded(
                  child: TextField(
                    controller: _billServiceCtrl,
                    decoration: const InputDecoration(
                      labelText: "Service (e.g. Sonography)",
                    ),
                  ),
                ),
                const SizedBox(width: 8),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isLoading
                    ...
                    : const Text('Save Record & Generate Invoice'),
              ),
            ),
          ],
        ),
