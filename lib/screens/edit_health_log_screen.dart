...
class EditHealthLogScreen extends StatefulWidget {
  final HealthLog log;
  const EditHealthLogScreen({super.key, required this.log});

  @override
  State<EditHealthLogScreen> createState() => _EditHealthLogScreenState();
}

class _EditHealthLogScreenState extends State<EditHealthLogScreen> {
  late String _logType;
  late double _painLevel;
  late String _bleedingLevel;
  ...
  void initState() {
    super.initState();
    _logType = widget.log.logType;
    _painLevel = widget.log.painLevel.toDouble();
    _bleedingLevel = widget.log.bleedingLevel;
    _moodCtrl = TextEditingController(text: widget.log.mood);
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await context.read<HealthLogProvider>().updateLog(widget.log.id!, {
        'log_type': _logType,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('✅ Log updated!'), backgroundColor: Colors.green));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Edit Health Log')),
      body: SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _card('Mood', Icons.mood, const Color(0xFF7C4DFF), TextField(controller: _moodCtrl, decoration: const InputDecoration(hintText: 'How are you feeling?'))),
        const SizedBox(height: 16),

        _card('Notes', Icons.notes, Colors.teal, TextField(controller: _notesCtrl, maxLines: 3, decoration: const InputDecoration(hintText: 'Notes...'))),
        ...
        SizedBox(width: double.infinity, child: ElevatedButton.icon(
          ...
          label: Text(_isLoading ? 'Updating...' : 'Update Log'),
        )),
        const SizedBox(height: 24),
      ])),
    );
